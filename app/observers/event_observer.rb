class EventObserver < ActiveRecord::Observer
  ...
  def check_alarms(event)
    ...
    Messenger.forward(event.place, longitude, latitude, address, event.geofence_warnings) unless event.geofence_warnings.empty?
    event.speed_warnings =  SpeedWarning.check(account.speed_alarms, event)
    Messenger.forward(event.place, longitude, latitude, address, event.speed_warnings) unless event.speed_warnings.empty?
  end
